...
- Are inconsistent in function and argument naming and documentation
...
But first, let's fix the `Gender` column to be called `Sex`, since it denotes biological sex (presence of a Y chromosome).
...
Let's make a base plot object to work from, with two discrete variables:
...
We'll be using the `scale_` functions for this.
There are many of these functions and they all have a consistent naming:
`scale_AES_SETTING()`.
For instance, 
we are using `fill`, so it would be `scale_fill_`.
And since we are using the viridis scheme,
it would continue to: `scale_fill_viridis`.
Lastly, because we have discrete variables, 
we need to tell viridis by adding a `_d` (for discrete) to the end:
`scale_fill_viridis_d()`.
This function has several color schemes within it, which
we can choose by using the `options` argument (it goes from `"A"` to `"E"`).
...
If we use the ColorBrewer scheme, it would be: `scale_fill_brewer()`.
This function also has several colour palettes,
which are viewable by using `RColorBrewer::display.brewer.all()`. 
The arguments `type` and `palette` in `scale_fill_brewer()` set the palette.

```{r}
# change colors to a ColorBrewer color scheme
base_barplot +
    scale_fill_brewer()
# change colors to another ColorBrewer color scheme
base_barplot +
    scale_fill_brewer(type = "qual")
# change colors to yet another ColorBrewer color scheme
base_barplot +
    scale_fill_brewer(type = "qual", palette = "Set2")
```

To use the colour schemes on a continuous variable, 
we would have to do a few things.
First, we'd most likely be using the `colour` aesthetic,
which means the scale would be `scale_colour_`.
To tell viridis to use the continuous scale,
we'd add a `_c` so it would be: `scale_colour_viridis_c()`.
For ColorBrewer,
the only difference is using `_distiller` instead of `_brewer`.
So it would be `scale_colour_distiller()`.
...
For adding lables to things, such as axis titles, the function is `labs()`.
