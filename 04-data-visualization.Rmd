...
There are many many ways to visualize additional variables to a plot 
and further explore your data.
We can use ggplots colours, shapes, sizes, transparency ("alpha"), 
and fill aesthetics, as well as "facets".
Facets in ggplot2 are was of splitting the plot up into multiple plots 
...
Let's visualize a few examples. Let try to answer a few questions:
...
For this plot, we'd maybe use `geom_boxplot()`. 
...
and how does age relate to these variables?
Here we have three continuous variables (`BMI`, `BPSysAve`, and `Age`),
so we could use something like `geom_point()`.
...
*Question*: How does BMI relate to systolic blood pressure 
and how does sex relate to these variables?
In this case, we have two continuous and one discrete (`Sex`).
We could use `geom_point()`:

```{r}
nhanes_tidied2 %>% 
    ggplot(aes(x = BMI, y = BPSysAve, colour = Sex)) +
    geom_point()
```

But with this plot, 
it is difficult to make out how sex may be related to these two variables.
We can add `geom_smooth()` to see how statistical curves differ between sexes.
And, we can add this geom on top of `geom_point()` by adding another layer:
...
This one is really hard to see what's different. 
But there is another way of visualizing a third (or greater) variable:
with "faceting".
Faceting splits the plot up into multiple subplots
and we do that with `facet_grid()`.
...
on it from the [viridis] package. 
...
Let's make a base plot object to work from that has two discrete variables:
...
This function has several color schemes within it 
and that we can change by using the `options` argument (it goes from `"A"` to `"E"`).
...
as well as setting the axis labels and plot title.
...
For adding titles to things, the function is `labs()`.
