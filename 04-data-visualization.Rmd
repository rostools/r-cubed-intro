...
since it denotes biological sex (presence of a Y chromosome).

```{r fix-sex-column}
# fix sex column header
...
ggplot(nhanes_tidied, aes(x = Sex)) +
    geom_bar()
```
We can see that for the most part there are equal numbers of females and males.
...
1. Like we did above, remove individuals that have missing diabetes status
and rename the `Gender` column to `Sex`. 
...
In this case, we have two continuous and one discrete variable (`Diabetes`).
...
We can also stack by `SurveyYr` and use `Age` as a colour:

```{r}
# Plot BMI in relation to systolic blood pressure, age, survey year and diabetes status using faceting
nhanes_tidied2 %>% 
    ggplot(aes(x = BMI, y = BPSysAve, colour = Age)) +
    geom_point() +
    facet_grid(cols = vars(Diabetes),
...
There are two: the viridis and the [ColorBrewer] colour schemes. 
Both are well designed and are colour-blind friendly.
For this course, we will only cover the viridis package.

## Exercise: Playing around with colour schemes

Time: 10 mins

Try changing a bar plot into 1-2 different colours to see how the functions work.

1. Let's make a base plot object to work from, with two discrete variables. Copy and paste the code below into your script file.

[ColorBrewer]: https://colorbrewer2.org/

```{r}
# Barplot to work from, with two discrete variables
base_barplot <- nhanes_tidied2 %>% 
    ggplot(aes(x = Diabetes, fill = Sex)) +
    geom_bar(position = position_dodge())
```

2. Now, use the `scale_fill_` function to add the colour scheme. Remember to use the `help()` or `?` functions in RStudio if you are wanting more information or documentation about  various `scale_` functions. We will use the viridis scheme to account for colour-blindness. We would need to indicate that we have discrete variables by adding a `_d` to the end of the function, and then we can choose from one of the colour schemes by entering a selection from `"A"` to `"E"` as an option. Copy and paste the code below into your script file.

```{r}
# change colors to a viridis color scheme
base_barplot +
    scale_fill_viridis_d()
# change colors to another viridis color scheme
base_barplot +
    scale_fill_viridis_d(option = "A")
```

3. Try changing the colour scheme by entering another selection (e.g., `"B"`, `"C"`, or `"D"`).

4. Let's practice using the colour schemes on a plot using continuous variables. First, we'd most likely be using the `colour` aesthetic, which means that the scale function would be `scale_colour_`. We'd also need to add a `_c` to indicate that we have continuous variables. Copy and paste the code below into your script file.

```{r}
# scatterplot to work from, with two continuous variables
base_scatterplot <- nhanes_tidied2 %>% 
    ggplot(aes(x = BMI, y = BPSysAve, colour = Age)) +
    geom_point()
# change colors to a viridis color scheme
base_scatterplot +
...
5. You can modify ggplot2 colour schemes using many other pre-defined palettes by installing new R packages, including scientific journal colour palettes ("ggsci") and even a Wes Anderson colour palette ("wesanderson")! Feel free to take some time after today's course to search online for other colour palettes that you think would suit your own work better. Remember to refer to the Data Visualization book in the [resources](resources) chapter.
...
- ggplot2 has 4 levels of grammar: `aes()` (what data to plot), `geom_` (e.g. `geom_bar()` what kind of plot), `scale_` (making the plot pretty) and `theme()` (controls the surroundings of the plotting area)
- Only use bar plots for discrete values - if applying them on continuous variables, it hides the distribution of the data  
- Use `position_dogde()` to get bar plots next to rather than on top of each other 
...
- Decide which graphs to include in the report and presentation.
