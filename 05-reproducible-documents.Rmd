...
insert_video("images/rstudio-create-rmarkdown.mp4")
```
...
## Summary of session
- Make reproducible research to become more efficient, productive and get more confident results
- Use R Markdown to construct files that can easily be turned into a variety of files as HTML or Word
- Insert r chucks in R Markdown document and chose to show actual code or just result in final document 
- Make tables by using `knitr::kable()`
- Apply headers ( `# Header 1`), text formatting (`**bold**`), lists (`-`), quotes (`<`), links `([Link](www))` and footnotes `([^1])` directly in R Markdown file 
- Insert pictures directly in file (`![Caption text](path/to/image.png)`) or `knitr::include_graphics(path/to/image.png))`, including `here::here()` in pathname
- Select topic to personalise your R Markdown  
