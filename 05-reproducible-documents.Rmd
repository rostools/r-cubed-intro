...
- Why
    - To use for exploration and documentation
    - To easily update reports on analyses and then share to collaborators
...
R Markdown is a file format (though still plain text like R scripts or `.txt` files)
that allows you to be more reproducible in your analysis 
and more productive in general in your data analysis.
R Markdown is a extension of [Markdown][pandoc] that combines R code 
and Markdown formatting markup. 

So what is Markdown? 
It is a [markup syntax] and formatting tool, like HTML,
that allows you write in plain text a document 
that can be converted to a vast range of other document types 
...
a very powerful and well-maintained software tool for document conversion. 
But, we'll get to Markdown a bit later,
for now we're going to focus on the main reason to use it:
for including R code and output into a document!

[markup syntax]: https://en.wikipedia.org/wiki/Markup_language
[pandoc]: https://pandoc.org/index.html


Output from R code gets inserted into the document for a seamless integration
...
Text formatting is done using special characters (like
