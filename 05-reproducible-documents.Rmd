...
This area where you typed is for code chunk labels.
In this case, we labeled the code chunk with the name `setup`.
...
there can be unintended side effects that you may not find 
or R may not tell you about 
that will cause you quite a bit of annoyance,
without really knowing the reason of what is going on.

[chunk labels]: https://yihui.name/knitr/options/#chunk-options

The name `setup` is also a special name for R Markdown.
Whenever you run other code chunks in the document 
and if you just opened up the document,
