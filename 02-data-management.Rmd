...
2. "Tidy data" is a conceptual framework that allows you to easy build off 
...
chaining them together in "sentences" (in a sequential pipeline), 
...
example_wide %>% 
    knitr::kable(caption = "Example **wide** dataset that is useful for data entry.")
```
...
example_wide %>% 
    ...
    knitr::kable(caption = "Example **long** dataset that is more usable for analyses and visualizing.")
```
...
like you would use in `select()` (e.g. can use the `select_helpers` like `starts_with()`,
or `-` minus to exclude).
