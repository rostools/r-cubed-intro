...
We've now covered the basic "verbs" (functions) of the dplyr package.
We'll now cover the "pivot" functions from dplyr's companion package, tidyr. 
There are many useful functions in the tidyr package, 
but the pivot functions (`pivot_longer()` and `pivot_winder()`) are key ones.
The simple action that pivoting does is to move columns over as rows, 
or rows as columns.
Another way to say this is that pivoting converts wide data into long data
or vice versa.

Wide data is data where values may repeat across columns. 
For repeated measurements, often it is easier to enter data in wide form,

A For example, sometimes we have data
...
knitr::kable(table4b, caption = "Example wide form dataset.")
```
...
NHANES %>% 
    select(SurveyYr, Gender, starts_with("BP")) %>% 
    gather(Measure, Value, -SurveyYr, -Gender) %>% 
    group_by(SurveyYr, Gender, Measure) %>% 
