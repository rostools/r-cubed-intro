...
- Use the R package `tidyverse` to tidy up data 
- Never edit raw data - instead, save the changes in a separate folder or even file based on a code, nor manual editing 
- Use functions from the `tidyverse`-package to `select()`, `rename()`, `filter()`, change or `mutate()`, `arrange()` and `summarise()` your data
