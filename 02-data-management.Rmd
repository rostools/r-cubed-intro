...
Now the column has been renamed from Gender to Sex. What if you want to select some columns and then rename some of them,
...
nhanes_small %>% 
    colnames()
# Shortcut for the pipe is Ctrl+shift+m
```
...
nhanes_update <- nhanes_small %>%
    mutate(Height = Height / 100,
           Logged_height = log(Height),
           Highly_active = if_else(PhysActiveDays >= 5, "Yes", "No"))
```
...
2. Using the large NHANES dataframe, create a new variable called `Urine_vol_average` by calculating the average
...
# Exercise: Try using the `summarize()` function on your own

Time: 5 mins 

Select another variable from NHANES to derive a summary statistic from, perhaps something other than min or max as we have just done). If you are unsure which summary staistics you can compute, consider using the `help()` or `?` functions to access documentation find help. Remember to use your `colnames()` and/or `str()`/`glimpse()` functions to take a quick look at the NHANES dataset and its variables.

Here's another example, if you get stuck:
...
# Exercise: Try using the `group_by()` and `summarize()` functions on your own

Time: 5-7 mins

To complete this exericse, we would like you to:
1. Select a new demographic and clinical variable from the NHANES dataset. Try to pick variables that we haven't used previously.
2. Compare diabetes status groups on these variables, using summary statistics of your choice. 

Note: You are likely to come across some errors in this exercise, so be sure to refer to your troubleshooting and help-finding materials.
...
1. *As a group*, complete item 2 of the [group assignment](assignment)
(to jump quickly to the assignment, 
run `r3::open_assignment()` in the RStudio Console).
2. *Individually*, open your group R project
...
3. *As a group*, complete item 4 of the [group assignment](assignment).
