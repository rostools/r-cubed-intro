...
**Tip**: When doing complicated wrangling 
and you finally get the data into a form that you will use often,
save the wrangled data as an "output" dataset in the `data/` folder.
...
Sometimes you only need certain columns from a dataset and not all of them. 
Or maybe you want to remove certain ones.
The `select()` function extracts specific columns from a dataset.


```{r}
# Select columns by name, without quotes
select(NHANES, Age)
