...
R scripts and Quarto files are shown), there is a "Document Outline"
...
button, use the keybinding {{< var keybind.outline >}}, go through the
menu to `Code -> Show Document Outline`. The Command Palette is a very
useful tool to learn, since you can easily access almost all features
and options inside RStudio through it. Because of this reason, we will
be using it a lot throughout the course. Open it up with
