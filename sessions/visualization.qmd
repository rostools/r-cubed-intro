...
Visualizing research data and especially the results of an analysis can
be an effective way of communicating your findings to others. In this
session, we will cover the basics of creating plots.
...
warning message, which tells us the problem: `glykemi` should be a
factor or a character for the `fill` aesthetic, but it is actually a
number. The `fill` aesthetic needs a categorical variable, not a
continuous one. We can easily fix this by using `as.character()` on the
`glykemi` variable in the `aes()` function.
...
we write code throughout this workshop. We'll use it with
