...
#| include: false
library(tidyverse)
post_meal_insulin_data <- read_csv2(
...
Our `post_meal_insulin_data` data frame is fairly tidy, though there are
a few things we will fix in later sessions. Rows are individual
participants at a specific time and the columns are the variables that
were measured, like weight. Let's visually explore our data! In the
`LearningR` project in the `docs/learning.qmd` file, create a new
second-level header on the bottom of the file called
`## Plot one continuous variable` and create a new code chunk below the
header with {{< var keybind.chunk >}}. Then we can make our first plot!
...
ggplot(post_meal_insulin_data, aes(x = BMI))
```

Run this code by using {{< var keybind.run >}}. You'll get a blank plot.
That's because we haven't told `{ggplot2}` what kind of plot we want to
make, which needs a `geom_` function.
...
#| fig-cap: "___"
#| label: fig-___
ggplot(post_meal_insulin_data, aes(x = ___)) +
  ___()
```
...
6.  Add a label in the `#| label` option section, again, replacing the
    `___` with a label that describes the plot.
7.  Replace the `___` under the code chunk where the `@fig-` reference
    is with the label you chose in the plot.
...
#| fig-cap: "___"
#| label: fig-___
ggplot(post_meal_insulin_data, aes(x = glykemi)) +
  geom_bar()
```
...
Run this code with {{< var keybind.run >}}. You'll see we get a warning
message. That's because the `glykemi` variable is actually a number, and
`{ggplot2}` needs a character vector for `fill`. We can easily fix this
by using `as.character()` on the `glykemi` variable in the `aes()`
function.
...
Running this code with {{< var keybind.run >}} will show you a barplot
of `Group` with `glykemi` as the fill, without the warning message. But
the legend on the side is a bit ugly. It's easy to fix with the use of
the `theme()` function, but we won't cover that in this session.
...
-   Naming issues: This issue is harder to fix and comes with experience
    and knowledge, but there are a few things:
    -   Programmatically wrong: The `T <- FALSE` is wrong because `T`
        ...
        though, since there are ways to identify and fix that). These
        issues can only be fixed manually.
    -   Semantically
-   Styling issues: This is much easier to fix and can largely be done
    automatically. But a few things:
    -   Spacing: The spacing around the brackets, commas, and math are
        inconsistent. Spacing is one of the most common stylistic issues
        in code, as the code will still run, but it is harder to read.
...
"Global Options", click the "Code" section, then click the "Formatting" pane,
and finally select "styler" from the drop-down menu list. Click ok and now,
whenever save your file, it will re-format it for you.
:::
...
#| filename: "docs/learning.qmd"
ggplot(post_meal_insulin_data, aes(x = BMI, y = auc_pg)) +
  geom_point()
```

Run this code chunk with {{< var keybind.run >}}. You'll see a
...
Run this code chunk with {{< var keybind.run >}} and we'll see a
