...
[resources](resources.html) page of the Guides website.
...
(`r palette_commit`) and push up to your GitHub repository.
...
code chunk. Complete as many tasks as you can below.
...
2.  With the `nhanes_exercise` data, use `geom_histogram()` to show the
    ...
    geom. Render (`r keybind_knit` or `r palette_render`) the document
    to make sure it works and to see the output.
...
3.  Using `geom_bar()`, find out how many people have data recorded for
...
4.  Using `geom_hex()`, find out how BMI relates to the two blood
...
5.  Using `geom_bar()`, find out how `phys_active` those with or without
...
6.  Using `geom_violin()`, find how `poverty` levels are different for
...
7.  Save, add, and commit the changes to the Git history.

```{r solution-2-var-plots, eval=FALSE, solution=TRUE}


# 2. Number of people who smoke now and are or above 40 years of age,
...
# 3. BMI in relation to mean arterial pressure
nhanes_full_tidied %>% 
