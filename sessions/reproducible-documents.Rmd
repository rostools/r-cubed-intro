...
1. Save the document in the `doc/` folder and name it `exercises.Rmd`.
...
**Note**, you can't use duplicate code chunk labels in your document.
...
from Section \@ref(group-by-summarise). But instead of using `phys_active`,
...
The first thing we should remove are the `NA` education rows, just like
we did with `diabetes`. Then, we'll 
convert it into a more elegant table in the R Markdown HTML output document,
...
1. Use `mutate()` to perform the following wrangling tasks:
    ...
    `round()`).
    - Use `str_to_sentence(education)` to capitalize the first letter of the words in the `education` column.
1. Rename `diabetes` to `"Diabetes Status"`, `education` to `Education`, and `mean_age` and 
...
- Finally, add and commit the changes you've made to the `doc/exercises.Rmd`. 
...
- Use headers ( `# Header 1`), text formatting (`**bold**`) and lists (`-`) in the R
Markdown file.
