...
will be called "Knit" with a yarn symbol beside it). To knit, you either
click that button or use the shortcut `Ctrl-Shift-K` anywhere in the R
Markdown document.
...
knit the document again with the "Knit" button or `r keybind_knit` (or
using the Command Palette with `r palette_render`). A Word document
...
code chunk (`r keybind_chunk` or `r palette_code_chunk`). The code chunk
...
@fig-image-rstudio-code-chunk-gear) or by typing them inside the code
chunk at the top. When adding them inside the code chunk, they always
need to start with `#|`.
