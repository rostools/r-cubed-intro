...
Palette by typing `Ctrl-Shift-P` and then typing "quarto" and select the
one that says "Create a new Quarto document". You can also use the menu,
by going to `File -> New File -> Quarto Document ...`, and a dialog box
will then appear. Enter"Reproducible documents" in the title field and
your name in the author field. HTML should be automatically selected as
the output format. There's also the option to use the "visual mode".
This mode is great if you are used to working with Word and you can test
it out on your own later. For this course, we will focus on using the
normal mode.
...
render the document again with the "Render" button or `r keybind_knit`
(or using the Command Palette with `r palette_render`). A Word document
...
created file into the Git history (`r palette_git_commit`).
...
`r keybind_chunk` or the menu item `Code -> Insert Chunk` to insert a
new code chunk. Like with everything else, you can also use the Command
Palette with `r palette_chunk`). Before we do that, let's delete all the
...
code chunk (`r keybind_chunk` or `r palette_chunk`). The code chunk
...
(`Ctrl-Shift-K` or `r palette_render`) the document and see what happens
...
Create a new code chunk (`r keybind_chunk` or `r palette_chunk`) and
...
Let's insert another code chunk below this one (`r keybind_chunk` or
`r palette_chunk`), and simply put `nhanes_small` in the chunk:
...
also render (`r keybind_knit` or `r palette_render`) the document and
...
a new code chunk with the label `mean-age-bmi-table`. Second, copy the
code we worked on from the Data Wrangling session, in
@sec-group-by-summarise, which is shown below for you to copy from.
Instead of using `phys_active`, let's change that to `education`.
...
more elegant table in the Quarto HTML output document, we use the
...
Now, render (`r keybind_knit` or `r palette_render`) and view the output
in the HTML document. Pretty eh! Before continuing, let's run `{styler}`
(`r palette_styler_file`) and then add and commit these changes into the
Git history (`r palette_git_commit`).
...
1.  In the `doc/learning.qmd` file, create a new header called
    ...
    `prettier-table`. Copy the code below (that includes some code we
    wrote above) and paste the code into the new chunk. Add the option
    `#| echo: false` to the code chunk.
...
5.  Run `{styler}` on the document (`r palette_styler_file`).

6.  Render the document to HTML (`r keybind_knit` or `r palette_render`)
    and see what the table looks like.

7.  End the exercise by adding, committing, and pushing the files to
    your GitHub repository (`r palette_git_commit`).
...
with the new picture. We can, if we want to, change the width and height
of the image as well as its alignment. We do this by adding a `{}` to
the end of the Markdown image tag and put options inside there.
...
(`r keybind_knit` or `r palette_render`) to see how the image changes.
...
-   Render the document (`r keybind_knit` or `r palette_render`) to make
...
-   Go to Quarto's [HTML
    ...
    (`r keybind_knit` or `r palette_render`).
-   Finally, add and commit the changes you've made to the document
    (`r palette_git_commit`). For now, don't add and commit the HTML
    output file.
