...
[GitHub](https://github.com/) (which this workshop is hosted on). In this
...
Very briefly reinforce the importance of using a PAT rather than
your own password. Also emphasize the use of a password manager.
:::
...
GitHub, so if you can, you should try to be more secure with what you send
...
GitHub from RStudio.
So, let's connect our projects to GitHub! Before we can link the projects'
Git repository to GitHub, we need to inform GitHub (authenticate) that
we are the owner of the account.

Very likely no one has set up a PAT, but let's check just in case
by running this function in the Console, which should give
an output similar to the below:
...
in the Console to create the token on GitHub (if one isn't set already).
...
bottom called "Generate token" and there will be a very long string
that's been generated that starts with `ghp_`. **Save this token in your
password manager**. Alternatively, leave this
page open to copy and paste the token whenever it's needed.
If this token gets lost, no worries! It's very easy create a new one by
re-running the function above.

::: callout-important
Do *not* save the token in the project we're working in for the workshop!
Save it somewhere else.
:::

This token we created is what we'll use every time we open
up RStudio and interact with GitHub through R. We do **not** need to
create a new token for each R project or package we make, we only need
to create one after our current token expires (typically every couple
of months), if we've forgotten the token or lost it, or if we've
...
goals of this workshop is to slowly move researchers more into the modern
...
will do for the group project at the end of this workshop.
...
video below). We won't do this for the workshop, but we're telling you how
you can just in case you want to after the workshop.
