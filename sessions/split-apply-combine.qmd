...
height by a group, you would use the argument `.by` inside
`summarise()`. Using `.by` will allow you to specify a grouping variable
that you want to split your data into. This will tell `summarise()` to
split the data up and apply an analysis, then immediately combine it
back together. These grouping variables are the columns names of the
columns that you want to group by These columns must contain categorical
data (e.g. education).
...
The `summarise()` function can be used on its own, but we can also use
it with the `.by` argument to specify the grouping variable.

{{< text_snippet hats_up >}}

## Calculating summary statistics

Let's try using `summarise()` without using `.by`. Go to the bottom of
your `docs/learning.qmd` Quarto file and create a new header called
`## Calculating summary statistics`. Then, create a new code chunk with
{{< var keybind.chunk >}}. Let's start with calculating the maximum
value of the BMI variable.
...
statistics. The real power of this function comes when you use it with
`.by`.

## Summary statistics by a group

While the `summarise()` function is useful enough on its own, it really
shines when combined with `.by`. Create a new header called
