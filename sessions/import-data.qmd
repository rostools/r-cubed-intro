...
for this workshop we will save it to the Git history to get practice using
Git. We also normally don't commit the generated `.html` files, but for
this workshop we will commit them to the Git history so that you get
...
already come up in this workshop: the `{tidyverse}`. The `{tidyverse}` is
