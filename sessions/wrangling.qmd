...
3.  Based on the columns from task 2 above, use `rename()` on those
    columns so that the `..` inside their name is `.minus`. You will
    need to type out these column names, both the old and the new,
    manually. Recall that renaming is in the form `new = old`.
