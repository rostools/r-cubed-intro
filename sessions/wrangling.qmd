...
Wrangling data into different forms and structures is one of the most
common tasks you'll do when working with data. In this session, we will
cover a "vocabulary" of the main wrangling tasks and use these "verbs"
...
| Sort rows | `arrange()` | Show rows with the smallest value at the top. We won't cover this function in this workshop, but we do have an extra section on it in @sec-extra-arrange. |
...
When you run this code with {{< var keybind.run-code >}}, you will see
that only the `Age` column is shown in the output.
...
columns to deal with, at least for this workshop. Let's keep only a few
columns for now, so it is easier to look at and explore.

Since we want this code to run every time we restart this Quarto
document, we should put it into the `setup` code chunk. Go to the
`setup` code chunk and at the bottom of it, add the following code:
...
then in the **Console**, check what it looks like by typing out the
newly created object there:
...
`rename()` function. Run this code with {{< var keybind.run-code >}} and
you will see the output with the new column name. We also have a column
...
3.  Based on the columns that was selected from task 2 above, use
    `rename()` on those columns so that the `..` inside their name is
    replaced with `_minus`. You will need to type out these column
    names, both the old and the new, manually. Recall that renaming is
    in the form `new = old`.
...
post_meal_data |>
  rename(
    P.Glucose_minus5.OGTT = P.Glucose..5.OGTT,
    Insulin_minus5.OGTT.X = Insulin..5.OGTT.X
  )
...
Run this by using {{< var keybind.run-code >}}. We'll see that more than
half of the participants are above overweight. Let's see how many are in
the `"CTR"` group, again, within the same code chunk.
...
`setup` code chunk. Go to the `setup` code chunk and at the bottom of
...
Run this code with {{< var keybind.run-code >}} to see the output and
see what it looks like now. Great! Before moving on, let's style our
...
Run this code with {{< var keybind.run-code >}} to see the new column.
To make use of this new data object, we should assign it to a new
object. Similar to the `small_post_meal_data` and
`tidier_post_meal_data` objects, we want these available to us easily,
so let's put this into the `setup` code chunk. Go to the `setup` code
chunk and at the bottom of it, write out the code below to create a new
data object.
...
Run this code with {{< var keybind.run-code >}} to see the new column.
We'll use this new data object in our next session.
