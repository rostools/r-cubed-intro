...
means insulin sensitive and a low value means insulin resistant. Let's
create a new header called `## Modifying columns` and then create a new
code chunk below that header with {{< var keybind.chunk >}}. We'll use
some math with the `/` symbol to divide values.
