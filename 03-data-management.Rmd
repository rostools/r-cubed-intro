...
## Final exercise: Combing all your knew dplyr knowledge

Time: 1 hour (?)

In this exercise, you have the chance to through the content of the whole dplyr section yourself and solve a more challenging exercise.


1. Create a new data-set where all column names start with lower case
2. Select columns Gender, Age and BMI
3. Make sure to exclude NAs from all selected columns
4. Rename gender to sex
5. Create a new column named age_class, where under 50-year-old named "under50", and over 50-year olds and olders are named "over50"
6. Group the data according to Sex and age_class
7. Calculate the mean and median BMI according to the grouping
...
# Create a new dataset with lower-case column names
nhanes_lc <- rename_with(NHANES, snakecase::to_snake_case)

nhanes_lc %>% select(gender, age,  bmi) %>% 
  filter(!is.na(gender) && !is.na(age) && !is.na(bmi)) %>% 
  rename(sex = gender) %>% 
  mutate(age_class = if_else(age < 50, "under50", "over50")) %>%
  group_by(age_class, sex) %>% 
  summarise(bmi_mean = mean(bmi, na.rm = T), bmi_median = median(bmi, na.rm = T))
```
