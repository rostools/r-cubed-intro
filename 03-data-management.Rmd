...
1. Write "tidier" and more readable code, with the pipe (`%>%`) operator.
...
We teach the tidyverse because of these abovementioned reasons. Here are some additional resources for the tidyverse:
...
- You should always save your origianl raw dataset in the `data-raw/` folder.
...
The argument after the dataset argument is `...`, 
...
Replace the `___` in the `select()` function, with the columns `tot_chol`, `bp_sys_ave`, and `poverty`.
...
4. Read through (in your head) the code below. How intuitive is it to read? Now, re-write this code so that you don't need to create the temporary
...
When a row in the `sex` column has the value `"female"`, that row is kept.
...
1. Filter `nhanes_small` so select only those participants with a BMI of more than or equal to 20 
...
Summarizing or applying simple (or complex) statistics to data is a key component of any analysis.
...
In [dplyr], you can use the function called `summarize()` to summarize on all the data.
...
Using `group_by()` splits the data up and `summarize()` applies an analysis, then immediately combines it back together. 
...
You would use `summarize()` to derive basic descriptive statistics of a certain
...
in combination with a `summarize()`, `mutate()`, `arrange()`, or other function.
However, the `summarize()` function can be used on its own. Let's take a look.
...
## Exercise: Using `summarize()` to calculate basic statistics
...
```{r solution-exercise-summarize-basic-stats, eval=FALSE, solution=TRUE}
# 1.
nhanes_small %>%
...
## Summary statistics by a group {#group-by-summarize}

While the `summarize()` function is useful enough on its own, it really shines when combined with `group_by()`.
...
This is simply a notification and can be ignored.
If you don't want the message displayed,
you can enter `options(dplyr.summarise.inform = FALSE)` at the top of your script
and run it.
...
## Exercise: Using `group_by()` and `summarize()` to answer statistical questions
...
Then, start replacing the `___` with the appropriate
code including `group_by()` with `summarize()`, to answer these questions:
...
females and males with or without diabetes?
3. Add and commit changes to the Git history.
