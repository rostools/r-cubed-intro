...
down and find the `to_snake_case()`. This is the function we want to use.
...
biological, which is what the data actually is.
...
*and then* renames the phys active column to physically active.
...
When used in `filter()` is would be used like:
...
$((2 \times DBP) + SBP) / 3$ (DBP = `bp_dia_ave` and SBP = `bp_sys_ave`) to calculate
...
save each new wrangled dataset you might create. But sometimes you'll need or
