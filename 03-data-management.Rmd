...
(it should be put in the `data-raw/` folder)
...
Table \@ref(tab:table-wrangling-verbs) below lists some common "verbs" from the [dplyr] 
...
tibble::tribble(
    ...
    "Calculate summaries of the data", "Calculating the maximum, median, and minimum age.", "`summarize()`",
    "Run an analysis by a group in the data", "Calculate means of age by males and females.", "`group_by()` with `summarize()`"
) %>% 
...
Let's add and commit the changes to the Git history with the RStudio Git Interface.
...
1. Add and commit the changes to the Git history with the RStudio Git Interface.
...
to read more about these functions and to get help to understand them. 
...
The full NHANES dataset is 10,000 individuals (rows) with 76 parameters (columns). Since we are only interested in some of these parameters, we will subset the large dataset and save it for future use as a new dataset.
...
your dataset may have column names that aren't very descriptive.
You'll probably want to rename them to something more explanatory, which is particularly important if you're sharing your work with others or in an environment where multiple people are working on the same data.
...
So, imagine that you want to select some columns and then rename them: do you then have to create a new data object every time? No! We can make use of a very powerful tool called piping (with the `%>%` function).
...
With `filter()`, when the logic conditions are equal to `TRUE`,
...
When used in `filter()`, the operators would be used like this:
...
3. Create a new variable called `young_child` where the age is less than 6 years.
4. Add and commit these changes to the Git history with the RStudio Git Interface.
...
If you want to do a split-apply-combine analysis (e.g. find the max height of females and males),
...
The `group_by()` function doesn't do anything by itself so should always be used in combination with `summarize()`, `mutate()`, `arrange()`, or another function. However, the `summarize()` function can be used on its own. Let's take a look.
...
you'll get a message informing you that R is `regrouping output`.
...
We get a warning about missing values begin present in the *diabetes*-column, 
...
## Exercise: Use `group_by()` and `summarize()` to answer statistical questions
...
We are going to take a pitstop here to practice what you learned earlier \@ref(01-project-management.Rmd) about troubleshooting errors, including going back to the start of the code and running each line/part one at a time until you find where the problem occurs.

Here is a piece of code similar to the one we were just using. When it is run in RStudio, it returns an error message. However, we can’t immediately see what the problem is, and we need a hint to locate where in the code the issue lies. We are going to try the one-line-at-a-time approach.
...
Here is where the error occurs, which indicates that the issues lies somewhere between the `group_by()` and `summarize()` functions. If we take a closer look, you can see that the pipe was left off after the `group_by()` function line.
...
For many projects, it isn't necessary or advisable to save every data object
...
3. If the dataset is a `.rda` file, use `load()`:
    ...
    With this code, you load the dataset and use it again, 
...
- Use the R package [tidyverse](https://www.tidyverse.org/) to load in multiple packages to tidy up the dataset.
- Never edit raw data - instead, save the changes in a separate folder or even a separate file. Save the data based on an R code, and don't manually edit the data.
- Use the functions `select()`, `rename()`, `filter()`, `mutate()` ("change or modify"), `arrange()` and `summarize()` your data from the [dplyr] package.
- Use the pipe (`%>%`) to write an easy-to-read code, similar to reading a text consisting of multiple sentences.
