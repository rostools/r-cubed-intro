...
more productive in general in your work. R Markdown is an extension of
...
so showing others how the analysis is done is easy 
...
there is a button with the word "Knit" and a yarn beside it, 
as shown in Figure \@ref(fig:image-rstudio-knit-button). 
To knit, you either click that button 
or by using `Ctrl-Shift-K` anywhere in the R Markdown document.
...
Before doing the exercise, add and commit the newly created R Markdown file
...
This is a good place to put your `library()` calls or, in our case,
the function `source()` to load all the packages.
Let's add the code to load the packages and the dataset to the chunk:
...
1. Use `mutate()` to update:
    ...
    them, rounding the values to 1 digit (`digits` is the second argument of
    `round()`).
...
Put your affiliations and your university or institution as items in a sub-list.
...
We'll do it with Markdown in this session and next session will be with R code.
...
- Making your research reproducible not only improves your science, but also
...
files such as HTML or Word.
- Insert R code chucks in R Markdown and automatically include the results in
the final document.
- Make tables by using `knitr::kable()`
- Apply headers ( `# Header 1`), text formatting (`**bold**`), lists (`-`),
and links `[Link](www)` directly in the R
Markdown file.
- Insert pictures directly in R Markdown with `![Caption text](path/to/image.png)` 
or `knitr::include_graphics("path/to/image.png"))` in a code chunk.
- For HTML, choose different themes for appearance to personalise your R
