...
```
3. Start a new R project:  Click the New Project button. We can either start a new project from scratch by clicking the button. If we click the right half of the button, we get the option to create a project by cloning a GitHub repository (more on that later):
...
- **Uploading files to RStudio Cloud projects:** To make files available to RStudio Cloud projects, we have to upload the files to it, Thankfully, this is very easy! In RStudio Cloud the "Files" tab of Panel D (ref. Figure \@ref(fig:rstudio-introduction)) contains a new icon "Upload File", which allows us to easily upload a file from our local computer (or several files within a .zip-file) to the RStudio Cloud folder we're currently in:
...
```
Of course, another way of loading files to RStudio Cloud is to synchronize the project with a remote repository, which will download all the tracked files to the RStudio Cloud project.
