...
you as an audience member will participate in the discussion by:
...
basic functions on. We don't want you to spend too much time on this task.
Here are some example datasets:
    ...
    - [Zindi.africa: Sea Turtle Rescue] (https://gitlab.com/rostools/r-cubed/uploads/7a59cb5f9429382d006d61d7667c62d7/turtles.zip)
    - [Zindi.africa: Busara Mental Health] (https://gitlab.com/rostools/r-cubed/uploads/addb3aa2118734a85961470530543298/rural_kenya.zip)
    - [Zindi.africa: Female-Headed Households in South Africa](https://gitlab.com/rostools/r-cubed/uploads/d7c0124f26722adc9c5467f569133367/rsa_wards.zip)
    - [Starbucks Menu Item Data](https://www.kaggle.com/starbucks/starbucks-menu) (May be required to create a Kaggle account)

3. **Download the dataset** and put it into the `data-raw/` folder of your project. For the GitHub-linked datasets above, you should be able to click on "Raw" or "Download" buttons, then use the keyboard shortcut (`Ctrl-S`) to save the page (as a .csv file). If you have documentation on the dataset, e.g. variable definitions, put those files in the `data-raw/` folder as well.

4. **Create an R script inside `data-raw/` that cleans up and prepares the raw dataset**
(as covered in [Data Management](wrangling.html)).
Save the new dataset in a folder called `data/`.

5. **Put the project under Git version control, then add and commit all the files**. 
Add your assigned GitHub group repository to your project 
and push ("upload") the repository up to GitHub
(covered in [Version Control](version-control.html)).
    - Update the README with the information from all of your team members 
    and with a short description of your project.
    - Make sure to add the open dataset to your Git repository and upload it to GitHub.

6. **Create an R script in your `R/` folder that generates 
and saves one or more figures** that visualize the cleaned dataset 
(covered in [Data Visualization](visualization.html)). 
Save the figures in a folder called `images/` inside the `doc/` folder.

7. **Create an R Markdown file** named `report.Rmd` in the `doc/` folder of your project
...
8. **Generate an HTML of the report** (but don't commit it to Git).
