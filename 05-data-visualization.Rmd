...
Open the previously used R Project, and create a new R Markdown file called
...
format. Delete un-needed text and code chunks, then save this file as
`visualization-session.Rmd` in the `doc/` folder. We will use this file for
the code-along and exercises in the data [visualization session] (visualization).

First, we want to load packages and dataset, so let's write that at the top of
the file, right under the YAML header.

`# Load packages and dataset`

Then add a new code chunk with the shortcut `Ctrl-Alt-I` or by using the menu
item `Code -> Insert Chunk` Then add the below code to the first code chunk:
...
In this session, we'll create a new code chunk for each plot to maintain a nice,
readable code, and to practice writing headers and inserting code chunks. So,
create a new code chunk and type this code:
...
(the default uses LOESS regression)

Let's check out how BMI may influence cholesterol using a basic scatterplot,
a hex plot, and a smoothing line plot, by inserting the code in a new code chunk:
...
However, the boxplot is still hiding your actual datapoints. These can be shown with a jitter plot:
...
Complete as many of the below tasks as you can.
...
What can you say about most people for these variables?
    ...
    b. `phys_active` (does the participant do moderate to vigorous physical activity)
...
The most common and "prettiest" way of adding a third variable is by using colours.
Let's try to answer a few questions by visualizing some examples.
...
You just need to write the name of the newly created file,
as well as the folder location (separate folders with `/`).
The next argument is the plot you want to save.
To set the dimensions of the figure, you can use `width` and `height` arguments.
...
- Use the "Grammar of Graphics" with the `ggplot2` package to plot your data.
- Prioritize plotting raw data data instead of the mean whenever possible.
- `ggplot2` has four levels of grammar: `aes()` (which data to plot), `geom_` (e.g. `geom_bar()` which describes the type of plot), `scale_` (making the plot pretty) and `theme()` (controls the surroundings of the plotting area).
- Only use bar plots for discrete values - if using continuous variables, it hides the distribution of the data as it is plotting the mean.
- You can plot three dimensions by using the X and Y axis  and colour - to plot even more dimensions, you can use `facet_grid()` .
- Use colourblind-friendly colours, e.g. the colour palettes `viridis` or `ColorBrewer`.
