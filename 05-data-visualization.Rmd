...
```{r continuous-variables, message=FALSE}
...
```{r barplots-deceive, fig.cap="Bars deceive what the data actually look like. Image sourced from a [PLoS Biology article](https://journals.plos.org/plosbiology/article?id=10.1371/journal.pbio.1002128).", echo=FALSE}
...
```{r 2-continuous-variables, message=FALSE}
...
2. Viridis has several palettes. Add the argument `option = "A"` to the
`scale_fill_viridis_d()` function. Run the function again and see how the colour
changes. Next, change `"A"` to `"E"`.
...
4. Similar to #2 above, use the `option` argument to set the palette to `"B"` 
...
```
#Has the images folder been created? I don't have it?
