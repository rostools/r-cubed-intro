...
a nice readable code, and to practice writing headers and inserting code chunks. Write out a new header called `# One variable plots` in the free text area after the setup code chunk. Then add a new code
...
Now, we'll do the same for the diabetes status variable. In a new code chunk, type:
...
First, enter a new Markdown header called `# Plotting two variables` and create the 
...
To try a more suitable `geom_`, add a new code chunk and type:
...
Now add a new code chunk and type:
...
geoms by adding another one with a `+`. In a new code chunk, type:
...
2. Viridis has several palettes. Add the argument `options = "A"` to the
...
4. Similar to #2 above, use the `options` argument to set the palette to `"B"` 
