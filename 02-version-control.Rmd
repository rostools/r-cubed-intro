...
```{r image-version-control-comics, echo=FALSE, fig.cap="'Final' version of a document, using a very common and highly manual 'version control system'."}
knitr::include_graphics(here::here("images/version-control.jpg"))
```

```{r image-filenaming-version-control-comics, echo=FALSE, fig.cap="Filenaming in a commonly used 'version control'."}
...
The basic workflow using Git is more or less to:

- Set up Git in your project or folder by creating it as a "repository".
...
and then click the "Commit..." option (or use the shortcut `Ctrl-Alt-M`).
...
2. These are the "Push" and "Pull" buttons that are what synchronize with GitHub, 
...
```{r image-git-states, echo=FALSE, fig.cap="The three states files and folders are in when using Git.", out.width="60%"}
...
and commit the change. 
Go back to the history and you should see the two commits done in your repository.
...
```{r image-git-remotes, echo=FALSE, fig.cap="The 'remote' vs 'local' repository, or online vs on your computer.", out.width="50%"}
...
```{r image-github-link-existing-repository, echo=FALSE, fig.cap="Code to copy and paste to link and upload an existing Git repository in GitHub."}
...
```{r image-git-remotes-synch, echo=FALSE, fig.cap="Synchronizing with GitHub: 'Pushing' and 'pulling'.", out.width="40%"}
...
Save, then open the Git interface,
by hitting `Ctrl-Alt-M` anywhere in RStudio 
(but not when in the Terminal) or going to the `Git button -> Commit`,
then add to the staging, and finally, commit the new change.
...
```{r image-git-remotes-collaborate, echo=FALSE, fig.cap="Collaborating with others using Git and GitHub by having a shared central GitHub repository.", out.width="60%"}
...
a merge conflict occurs that you must deal with manually.
    ...
    `=======`, and `>>>>>>> origin/master` 
    (the last name may sometimes look different).
