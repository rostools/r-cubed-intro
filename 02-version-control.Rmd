...
```{r image-filenaming-version-control-comics, echo=FALSE, fig.cap="Filenaming used for the common 'version control'."}
...
- Have you ever forgotten what you were doing on a project, or why you chose a particular strategy?  
...
While many people may use Git to manage their R Projects, you can also work with other non-code file types in Git. In the previous section, you learned that you can load in datasets of many file types, including `.csv` and `.rda` files. You can also import image files, `.zip` files, Word documents, and PDFs. Version control is useful for any project as any type of documentation can be saved in the Git history, which allows for the ability to revert to an earlier version if necessary. You can save files either by adding and committing them or by uploading them to your repository on GitHub, as pictured below.
...
```{r image-rstudio-git-button, echo=FALSE, fig.cap="The Git button location in RStudio."}
...
1. The "Changes" and "History" buttons that allow you to switch between views.
...
2. The "Push" and "Pull" buttons are what synchronize with GitHub, 
which we will cover later in the session.
3. The panel that lists the files that have been modified in some way.
You add ("stage") files here that you want to be put into the history.
4. The Commit Message box, 
where you write the message about the changes that will be put into the history.
5. The panel that shows what text has been modified, added, 
...
Changes are detected at the line level (what line in the file).
...
Tip: If you have a lot of files to stage and you don't need to review each change: You can stage them all in a single click by pressing CTRL-A to select all files (⌘-A for Mac users) and then clicking the "Staged" checkbox one of the files. 
...
```{r image-git-states, echo=FALSE, fig.cap="The three states files and folders are in when using Git."}
...
(*note*: "Ctrl+V" does *not* work) and press Enter.
...
```{r image-rstudio-terminal-tab, echo=FALSE, fig.cap="Location of Terminal in RStudio."}
...
```{r image-git-states-with-github, echo=FALSE, fig.cap="Which states get pushed and pulled."}
...
by typing "Ctrl+Alt+M" anywhere in the RStudio interface
...
You should see something like the below text (may be slightly different):
...
without writing a commit message (this is not necessary when resolving merge conflicts)
Push the changes to GitHub, then open up the Git repository in GitHub,
refresh the browser, and view that the changes have taken place.
...
But you will get practice collaborating on a Git project in the final exercise.
...
GitHub is a popular remote Git repository hosting service.
...
1. During the project management session's final exercise on the first day, 
...
and upload (push) their project files to GitHub.
    - Use the process we did *for an existing repository*.
2. After the coordinator has done this, 
each team member must "clone" (download) 
the updated project using RStudio's interface
("File -> New Project -> Version Control -> Git").
    - Use the process we did *for new repositories*.
3. Then each team member needs to move any files they worked on in their old project folder
and move them into the newly created Git repository R project.
Then add to the staging, commit, and push those file changes to GitHub.
    - You'll likely have to deal with Merge Conflicts at this point. 
    We'll be here to help out.
4. Then complete item 5 and its sub-tasks in the [group assignment](assignment.html)
