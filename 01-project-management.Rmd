...
Let's learn a bit more about how R works, and what the "R session" means. An R
...
isn't where you actually work... you would work in the folder that has your R
scripts or data files. With R Projects, this assumption is implicit because the
R Project contains your R scripts and data files, and it is this folder location
that the R session opens in.

Back to the project we created. There are README files in each folder that
...
`_` or `-` are recommended to be used instead of a space, thought using `-`
...
```{r solution-better-filenaming, eval=FALSE, echo=FALSE}
# Bad: Has a space.
fit models.R
...
```{r solution-ex-wrong-code-naming, eval=FALSE}
# Object names
...
But! When you `source()` an R script, the R code may likely run *in the
...
```
LearningR <--- R session working directory starts here.
├── R
│   ├── README.md
│   ├── fetch_data.R <--- Working directory when running from source()
│   └── setup.R
├── data
│   └── README.md
├── doc
...
**Please take 5 mins to read this section.**

You *will* encounter problems and errors when working with R, 
and you will encounter them *all the time*. In fact, most of your time in R will be spent figuring out solutions to these errors ("debugging"). Error messages will appear in red text in your Console and start with "Error:". Warning messages that are sometimes harmless are also usually in red, so make sure to read if it says "Error" or not. This section is designed to give you some tips for how to work around problems, as you won't always have stickies and helpers in the room to come over to your computer, and help you work through things! Here are some initial steps to take when you encounter an error:
...
1. **Go back to the start of the code and run each line or part one at a time**, to see where the problem occurs. You will get an opportunity to practice this later, once you are working with bigger chunks of code.
...
```{r exercise-errors, error=TRUE}
x < y
Library(tidyvrese)
```

```{r solution-exercise-errors, eval=FALSE, echo=FALSE}
x < y
library(tidyverse)
```
