...
workspace (all the R activity and output is temporarily saved), history, and
documents.
...
For this course, we'll need to delete the files `fetch_data.R` and `setup.R` in the `R/` folder,
as well as the `.Rbuildignore` file, since we won't need them.
...
(like `c()`). Others, like `head()` only can take a few arguments.
...
For instance, the object `weight_kilo` would have been named something like `x`.
...
You may have completed the exercise by hand, 
...
You see there's also a new thing `here`! The [here] package uses a function called `here()` 
...
encounter them *all the time*. In fact, a large amount of your time in R will be spent
figuring out solutions to these errors ("debugging"). Error messages will appear
...
telling you**. Some common error messages include:
    ...
    - *"Unexpected symbol in..."*: Usually is because you misspelled a variable
...
1. **Go over the code again and carefully check for any mistakes:**
    ...
    - Forgot to load or re-load your packages? Packages are automatically unload
    when you exit out of RStudio and R. So you need to load them in each new
...
The pieces of code below will return an error message due to one of the mistakes
noted above. You don’t need to run this code in the `R/project-session.R`, but you can copy and
paste the code into RStudio so that you can play around with it. Look at the
code, identify the mistake, and provide the corrected code. You don’t need to
