...
start with is a "blank slate". This is done by typing the following code in the Console:
...
(like `c()`). Others, like `head()` can only take a few arguments.
In the case of `head()`, the first argument is reserved for the data frame.
...
essential skill in the tasks and projects for a modern data analysts.
...
1. **Go over the code again and carefully check for any mistakes:**
    ...
    - Capitalized something that didn't need to be capitalized?
