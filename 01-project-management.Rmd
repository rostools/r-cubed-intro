...
**Please take 5-10 mins to read this section.**

You *will* encounter problems and errors when working with R, 
and you will encounter them *all the time*. In fact, most of your time in R will be spent figuring out solutions to these errors ("debugging"). Errror messages will appear in red text in your Console. This section is designed to give you some tips for how to work around problems, as you won't always have stickies and helpers in the room to come over to your computer, and help you work through things! Here are some initial steps to take when you encounter an error:

1. **First, try to stay calm**; problems happen to everyone, 
no matter their skill level. You *can fix it*! `r emo::ji("smile")`
1. **Stop to read through the error message and understand what RStudio is telling you.** Some common error messages are:
    - "Could not find function" – Usually means that you have misspelt the function, or an R package has not loaded properly
    - “Object not found” – Usually means that you have not initialized (saved) the object, or the object is initialized but empty 
...
1. **Go over the code again and carefully check for any mistakes:**
    ...
    - Forgot to load or re-load your libraries and packages?*
*Your libraries will automatically be unloaded every time you exit out of RStudio, and RStudio has been known to spontaneously unload your libraries during a session. As such, you will often have to check that all the libraries you are wanting to use have been loaded.

## Exercise: Quick fixes and troubleshooting

**Time: ~5 min**

The pieces of code below will return an error message due to one of the mistakes noted above. You don’t need to run this code in RStudio, but you can copy and paste the code into RStudio so that you can play around with it. Look at the code, identify the mistake, and provide the corrected code. You don’t need to understand what the code does.

```{r, eval=FALSE}
# Library()
Error message: could not find function “Library”
...
## Exercise: Where have we gone wrong?

You may have combed through your piece of code and not been able to find any small fixes. The next step is to go back to the start of the code and **run each line or part one at a time**, to see where the problem occurs.

Here is a piece of code that you will see again later in the course. When it is run in RStudio, it returns an error message. However, we can’t immediately see the problem and need a hint to locate where in the code the issue lies. We are going to try the one-line-at-a-time approach. Please note that, again, you don’t need to understand what the code does just yet.
...
Here is where the error occurs, which indicates that the issues lies somewhere between eggs and home. If we take a closer look, you can see that the pipe was left off after the “buy” function.

## Final troubleshooting tips

1. **Restart the R session (`"Session -> Restart R"` or `Ctrl-Shift-F10`)**. 
Then run the code from beginning again and track what objects get created, and if
the proper object name is used later on.
1. (Rarely need to do) **Close/ re-open RStudio and try again.**
1. **Use `help()` or `?` to access built-in documentation about a function or package.** When you run this function on an object,
it will open the relevant documentation in the "Help" pane of RStudio (bottom right-hand corner). **Try it out:** Enter either of the following commands into your console and Run.
...
Sometimes, this documentation can be hard to read and seem overly complex for a beginner. Instead, you can try finding the website for the package you are having trouble with, as they often have guides that are a little easier to understand. The [tidyverse](https://www.tidyverse.org/packages/) packages all have amazing documentation that you can use to help you with problems you may have.

1. **Check out the [RStudio cheatsheets](https://rstudio.com/resources/cheatsheets/)**, which are printable PDF files that are 
great sources of help and learning. You can access cheatsheets by select Help  Cheatsheets from the top menu bar. 
1. **Consider explaining the problem out loud to a colleague or friend.** You might find that, in stepping through the problem and explaining what it is that you are trying to do/solve, you will come up with the solution yourself.
1. **Take a break and come back to it!**
1. **Google it.** Chances are that someone has already encountered that error and has asked about it online. In fact, those who are “more expert” in coding languages like R are skilled mostly because they are strategic and know how ask Google the right questions. Sometimes, Googling the error message will be enough, but sometimes you will need to include “R” and the relevant package or function as a keyword in your search.
-If all else fails, you can always turn to the trusty **online R community. Check [StackOverflow](https://stackoverflow.com/questions/tagged/r),**, a coding-related question and answer website, to see whether your issue has already been asked about and solved by others. If it hasn’t and you are considering submitting a question, make sure to read the posting guides beforehand to ensure that you are asking the question in a helpful way. 
