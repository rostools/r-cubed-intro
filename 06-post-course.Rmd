...
- For the first issue, check our [resources for learning](resources#for-learning).
- For the second issue, the best approach is to practice what we preach, and explain what we practice. The reproducible work-flow is a way to help ourselves and improve productivity. The most common (perhaps only) reason to shy away from it is a lack of exposure and understanding. If our supervisor only uses Microsoft Word and gets offended whenever we say the word Git, we'll start with small exposures, e.g. knit our RMarkdown report to Word format and mail the .docx to him, and when we get an email back, save the document containing edits/feedback in a Git repository so we can version control it with Git. Even if it involves a bit of manual work, we still get some of the benefits of reproducibility and version control. Also, we'll be able to demonstrate in practice how reproducible documents and RMarkdown work, as well as Git for version control. In time, perhaps we can use this as a way to showcase to persuade our supervisor to consider using the tools himself.
- Regarding the third issue, it should not be an issue. We'll go over why and how:
