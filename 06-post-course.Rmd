...
A showcase of how the reproducible workflow we've gone over in the course is possible in a restricted virtual environment. Case in point: The virtual desktop environment used when working on Statistics Denmark's researcher services
...
We'll get a message nudging us to set up our git config and not just use the defaults. The defaults should be based on your log-on ID (and somewhat sensible) here, but we'll set them ourselves just for good measure (since we don't have access to the r3 package, we'll set it with the git2r package here):
...
We can still use git with a local repository on the virtual desktop to track our own work - and in a few steps, we'll set up a remote and get all the functionality of git!  
But first, let's set up git in the project:

```{r dst_git_setup, eval=FALSE}
usethis::use_git()
```
**And it works: See Figure \@ref(fig:dst-git-works)**
...
If you recall Figure \@ref(fig:image-github-link-existing-repository), you can create a remote repository from an existing repository by pushing it from the terminal command line. The only difference here is that the location of the remote is not a github-URL. Instead, we create it in a folder on the virtual desktop, that everyone can access and clone from:

> `mkdir -p E:/project_directory/git-remote/` # to create the folder for the remote directly from the command line (you can also do this through the File Explorer in Windows)
> `git remote add origin E:/project_directory/git-remote/remote.git`  
