...
Now, we make a custom function to do the file conversion and saving, and we feed it all the file names we stored in `file_list` with `purrr::walk()`, which takes the files and applies our custom function to each file.
...
my_automatic_conversion_function <- function(filename) {
    ...
    # It is possible to add code to filter the 
    # data here, if you want to trim the dataset 
    # during conversion
    saveRDS(dataset, paste(work_folder, filename, '.rds', sep = ''))
}

# walk runs a function on a vector (of file names)
# that only e.g. save a dataset.
purrr::walk(
    file_list,
    my_automatic_conversion_function
)
```
...
Next, we have to set up the connection driver, that is: We have to name it and specify the address of the database we want it to connect to, shown in Figure \@ref(fig:odbc-admin-2).
