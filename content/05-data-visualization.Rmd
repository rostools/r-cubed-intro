...
like age or BMI in ggplot2. 
For discrete variables like education status,
...
You'll notice we get a warning about dropping missing values. That's ggplot2 
letting us know we have some missing values. So, like with `median()` and
many of the other summary statistic functions, we can set `na.rm = TRUE` to
`geom_density()` and other `geom_*` functions.
...
There are many discrete variables in NHANES, including education and diabetes, 
...
We didn't use `na.rm = TRUE` here because `geom_bar()` includes that as a column.
...
Since both are important variables for us, let's remove all missing
...
Let's check out how BMI may influence systolic blood pressure using a basic scatterplot,
...
code chunk named `2-continuous-variables` below that. Like with the previous
plot we created using `bmi`, we'll use `na.rm` again.
...
Let's use this to see difference in diabetes status between education levels. Create a new code chunk named `2-discrete-variables` and type:
...
Complete as many tasks as you can below. Don't forget to use `na.rm = TRUE`
where necessary. Use the provided `nhanes_full_tidied` version when creating
plots with `ggplot()`.
...
and two discrete variables (`education` and `diabetes`).
...
Do you see differences in systolic blood pressure between the education statuses? Between diabetics and non-diabetics?
